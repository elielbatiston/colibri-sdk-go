...
// Query is a struct for MongoDB query
type Query[T MongoDBModel] struct {
	ctx    context.Context
...
func NewQuery[T MongoDBModel](ctx context.Context, filter bson.M) *Query[T] {
...
// filter: bson.M for filter
// Returns a pointer to Query struct
func NewCachedQuery[T MongoDBModel](ctx context.Context, cache *cacheDB.Cache[T], filter bson.M) (q *Query[T]) {
...
func (q *Query[T]) Many() ([]T, error) {
	return q.ManyInInstance(mongoDBInstance)
}
...
// fetchMany retrieves multiple items of type T for the given MongoDB instance.
//
// instance: The *mongo.Client instance to execute the query.
// Returns a slice of retrieved items of type T and an error.
...
func (q *Query[T]) One() (*T, error) {
	return q.OneInInstance(mongoDBInstance)
}
